"""Build the POV room (Blender 5.1, Cycles; scene content in room_scene.py).
  ...
  blender -b -P scripts/room/build_room.py -- preview --state night|day|screen
      [--view pov,wide,window,top,lamp,hands,handside,finger,mug] [--samples 96]
...
# White balance per state, as a camera would set it: at night the lamp stays
# warm without going orange; by day the blue skylight reads near neutral and
# the sun golden.
NIGHT_WB = 3800
DAY_WB = 7500
...
def setup_lights(h):
    d = LAYOUT["desk"]
    lamp = add_light("light_lamp", "SPOT", h["lamp_bulb"] + h["lamp_axis"] * 0.045, h["lamp_axis"],  # just outside the bulb glass
                     color=kelvin(2700, NIGHT_WB), spot_size=2 * h["lamp_half_angle"], spot_blend=0.6, shadow_soft_size=0.015)
    ...
    return {"lamp": lamp, "led": led, "sun": sun, "sun_dir": sun_dir, "street": bpy.data.objects["light_street"]}
...
def set_state(state, h, world, lights, levels):
    ...
    rs.emission_input(m["bulb"]).default_value = levels["bulb"] if night else 0
...
LEVELS = {
    "street": 12.0, "street_light": 6000.0, "lamp": 30.0, "bulb": 25.0, "led": 6.0, "led_strip": 10.0, "fans": 14.0,
    "night_sky": 0.02, "city_yaw": 150, "screen_preview": 1.0,
    "sun": 28.0, "sky": 32.0, "day_view": 2.3, "day_yaw": 150,
}
...
VIEWS = {
    ...
    "lamp": ((0.1, 0.95, 0.1), (-0.5, 1.25, -0.65), 60),
...
def render_environment(path):
    """Equirectangular panorama from the seated eye, for reflections and
    bounce light on the realtime hands and keyboard."""
    scene = bpy.context.scene
    ...
    cam.rotation_euler = (math.pi / 2, 0, 0)  # looking along -Z (glTF), the room's forward
    scene.camera = cam
...
def bake(h, world, lights, samples):
    ...
    for state in ("night", "day"):
        ...
        render_environment(OUT / f"room_env_{state}.hdr")
