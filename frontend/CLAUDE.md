...
- Anti-FOUC: `app/layout.tsx` renders both default palettes as CSS (`:root` dark, `@media (prefers-color-scheme: light)` light), so a first visit paints the default `auto` mode would resolve. Every persisted change also snapshots the full palette to `term-site:palette`, and the inline script in `app/layout.tsx` applies it before first paint, so saved themes survive navigations without flashing the default; `initialize()` rewrites an existing snapshot to the resolved theme so one saved under an older default stops flashing. Keep `paletteVars` and that script's variable list in sync with `applyThemeEntry`.
