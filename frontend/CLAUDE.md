...
- **Lighting contract** — static room meshes keep their PBR textures and take diffuse light from three Cycles lightmaps on `TEXCOORD_1` (`room.ts`): night, day, and the monitor alone (tinted each frame by the terminal's average colour). They are 8-bit WebPs on a log curve (`room.json` `lightmaps.range`; decoded in the shader) because the sRGB curve crushes the room's dim half into a few codes. The day lightmap omits the sun's direct light: the shadowed sun adds live diffuse everywhere so the blinds' shadow stays crisp, while the other realtime lights add only specular to those surfaces; the keyboard, mouse, arms and `live_*` props get the full realtime lighting: as environment light, at night a light probe each (`probes.ts`: the baked room rendered once at load from beside the object, so the LED-lit wall and hex panels light the props near them as the bake lights the wall; the lamp's LED and the monitor are left out, being realtime lights), by day the day panorama, the monitor as a `RectAreaLight`, and a shadowed lamp and sun placed and sized from `room.json` `lights`. Units match Cycles: a Blender point/spot of P W is intensity P/4π, a sun of strength S is intensity S. `room.ts` and `skin.ts` patch three's lighting chunks (skin: wrapped, scatter-tinted terminator, a second wider shadow lookup for red, exposure-style highlight compression, a larger normal bias against acne in the finger webs). `shadows.ts` replaces PCF's per-pixel-rotated taps (noise with no TAA to average it) with a tent filter over `shadow.radius` texels that compares each tap against the receiver's plane where it comes nearer the light, and by day adds `sunDetail`, a fine sun shadow map fitted around the desk, so the hands' shadows on the keys stay crisp. Glass reflects the room through its own `envMap` (`screen.ts` `PANEL_REFLECTANCE`, `devices.ts` `GLASS_REFLECTANCE`, set from `lighting()` via `setEnvironment`): three scales only a material's own `envMap` by `envMapIntensity`, and applies `scene.environment` at the full `scene.environmentIntensity`, which by day veils the light-theme text. `contact.ts` darkens the keyboard and mouse under spheres `hands.ts` publishes for the fingertips, finger joints and palms (diffuse by solid angle, specular by the reflection cone). Each patch `console.warn`s if a three upgrade changes the patched lines.
- **Desk decor** (`scripts/room/room_scene.py`, positions in `layout.json` `decorZones`) — the key light is the LED bar lamp at the desk's back left: its diffuser, rolled toward the keyboard and ahead of the monitor so the monitor never shades the keys, becomes `room.json` `lights.lamp` (a neutral-warm 3500 K under the 3800 K night white balance). The monitor hangs on a space-grey two-link arm whose pole hides behind the screen; from the seat only the elbow shows beside the screen's right edge. Six Nanoleaf-size hex light panels snake across the wall behind the screen and out past its right edge, two whole in the frame (their wall glow is baked from `light_hex*`; they are baked like the wall, mid-grey diffusers when off; a controller's lead drops straight down the wall behind the speaker); the glass-sided PC stands on the floor under the desk's right end. By day `proxy_fill`, an emitting plane (baked only), stands for daylight through the door behind the chair, so the wall right of the monitor isn't black beside the sunlit desk; the room's skylight is held below the street's so the desk's two halves sit within about a stop. `room_env_{night,day}.hdr` is rendered with camera rays seeing the sky that lit the bake (not the city photo shown through the window), so realtime-lit props match the lightmapped room by day, and centred on +X, the column three.js maps an equirect's centre to. A bias light on the monitor's back (`light_bias`, baked only) washes the wall behind it so the right of the desk reads as silhouettes at night. The keyboard is wireless and the earbuds charge on the phone's 2-in-1 stand; no device cable crosses open desk from the seat: the lamp's, clock's and laptop's run along the desk's back edge to one grommet behind the monitor's left side, the phone stand's into a small grommet hidden behind the phone, the macro pad's and mug coaster's under the mat and through the desk, the speaker's straight back and down behind the desk. Materials named `emit_*` (LED strip, monitor bias light, PC fans, lamp diffuser and power dot, mug LED, speaker glow, the wall's hex light panels) glow at their `room.json` `emitters` night/day strengths. Objects named `live_*` (the mug, the smart speaker) take no lightmap: they are small and curved, so the realtime lights shade them; their materials must not be shared with baked meshes, since `room.ts` patches lightmaps in per material. `engine/devices.ts` draws on three `rt_*` quads: `rt_clockFace` (a 64 × 32 pixel-matrix display with the visitor's local time and a contribution graph), `rt_phoneScreen` (the phone's always-on lock screen: a status-bar time over a now-playing card and a notification stack, so it never repeats the display's big clock) and `rt_deckKeys` (one face per raised key, each mapping its cell of a 5 × 3 canvas of key icons in the Nerd Font the keyboard already loads); the room fails to load without them. `atmosphere.ts` raises steam off the mug's `coffee` mesh when it exists.
