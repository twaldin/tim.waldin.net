...
1. `unsetopt PROMPT_CR` — no `%` partial-line marker — zsh completion (`compinit -C` over the `~/.zcompdump` the Dockerfile builds with `zsh -i -c exit`; generating it at session start cost ~0.9 s of the 0.5-CPU budget before the first prompt), and `bindkey -e` to keep Emacs-style command-line editing despite `EDITOR=nvim`.
