...
// `Cache-Control: immutable`) from a per-worker memory cache, for the
// "returning visitor" tests in terminal.spec.ts only. The "first visit" check
// deliberately runs without this so the cold-load signal stays observable.
//
...
// navigations; this restores that behaviour for the later tests. Each asset is
// still downloaded from the site under test (once per worker, plus the bounded
// 503 retries below), and every page load still spends live requests on the
// HTML, RSC prefetches, the pageview beacon and the socket, so a broken deploy
// is still caught.
const IMMUTABLE_PATH = /^\/(_next\/static|fonts)\//;

// Filling the cache is itself one cold burst, and it follows the uncached
// first-visit load, so it can graze the limiter; a 503 from it is transient,
// so back off and refetch. Only 503 is retried: nginx answers limiter
// rejections with 503, while an unreachable or broken frontend surfaces as
// 502/504 and still fails the test.
